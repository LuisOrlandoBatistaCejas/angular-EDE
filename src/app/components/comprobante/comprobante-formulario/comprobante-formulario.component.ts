...
@Component({
  selector: 'ms-comprobante-formulario',
  templateUrl: './comprobante-formulario.component.html',
  styleUrls: ['./comprobante-formulario.component.scss']
})
export class ComprobanteFormularioComponent implements OnInit, DoCheck {
     ...
     */
    onSubmit(comprobante: any) {
        if (!this.isNew) {
            comprobante.id = this.comprobante.id;
        }
        if (this.userCajero) {
            comprobante.Vendedor = this.appService.user.detalle;
        }
